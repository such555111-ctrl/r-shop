...
def cmd_start(message: types.Message):
    if not BASE_URL:
        bot.send_message(
            message.chat.id,
            "r!shop: мини-апп ещё не сконфигурирован (не задан WEBHOOK_URL)."
        )
        return
    ...
    bot.send_message(
        message.chat.id,
        "🛍️ *r!shop*\n\n"
        "Добро пожаловать в r!shop! Кроссовки, худи, футболки, кепки и "
        "аксессуары — трендовые товары для вас.\n\n"
...
def admin_message_customer(oid):
    ...
    try:
        bot.send_message(
            chat_id,
            f"💬 Сообщение от r!shop по заказу RS-{oid}:\n\n{text}"
        )
    except Exception as e:
...
def admin_message_question(qid):
    ...
    try:
        bot.send_message(
            chat_id,
            f"💬 Ответ от r!shop по товару «{q.get('product_name','')}»:\n\n{text}"
        )
    except Exception as e:
